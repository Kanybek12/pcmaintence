...
export default {
  ...
  methods: {
    saveData () {
      ...
      if (this.isForEdit) {
        ...
        this.store.updateReasonReference(body, this.selectedRow.id)
      } else {
        console.log('BODY ', body)
        this.store.createReasonReference(body)
      }
    },
    onEditClick (row) {
      console.log('ROW ', row)
  ...
  computed: {
    ...
    createReqDialogTitle () {
      return this.isForEdit ? 'Редактирование Причины' : 'Причина'
    },
    columns () {
      return [
        {
    ...
    reasonData () {
      return this.store.reasonList
    },
    rows () {
      this.reasonData.forEach((itm, i) => {
        itm.id = i + 1
      })
      return this.reasonData.filter(itm => {
