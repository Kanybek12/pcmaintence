...
export default {
  ...
  methods: {
    saveData () {
      ...
      if (this.isForEdit) {
        ...
        this.store.updatePriorityReference(body, this.selectedRow.id)
      } else {
        console.log('BODY ', body)
        this.store.createPriorityReference(body)
      }
    },
    onEditClick (row) {
      console.log('ROW ', row)
  ...
  computed: {
    ...
    createReqDialogTitle () {
      return this.isForEdit ? 'Редактирование Приоритета' : 'Приоритет'
    },
    columns () {
      return [
        {
    ...
    priorityData () {
      return this.store.priorityList
    },
    rows () {
      this.priorityData.forEach((itm, i) => {
        itm.id = i + 1
      })
      return this.priorityData.filter(itm => {
