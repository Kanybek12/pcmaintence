  ...
  <div class="mainContainer">
    ...
    <q-table
      ...
      <template v-slot:top>
        ...
        <q-input label="Поиск" dense outlined v-model="searchText" style="min-width: 150px; margin-right: 30px; margin-bottom: 20px;">
...
export default {
  ...
  methods: {
    saveData () {
      ...
      if (this.isForEdit) {
        ...
        this.store.updateOnholdReference(body, this.selectedRow.id)
      } else {
        console.log('BODY ', body)
        this.store.createOnholdReference(body)
      }
    },
    onEditClick (row) {
      console.log('ROW ', row)
  ...
  computed: {
    ...
    createReqDialogTitle () {
      return this.isForEdit ? 'Редактирование Ожидания' : 'Ожидание'
    },
    columns () {
      return [
        {
    ...
    onHoldData () {
      return this.store.onholdList
    },
    rows () {
      this.onHoldData.forEach((itm, i) => {
        itm.id = i + 1
      })
      return this.onHoldData.filter(itm => {
